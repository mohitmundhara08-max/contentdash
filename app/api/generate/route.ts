...
async function callAnthropic(
  ...
  if (!res.ok) {
    let message = `Anthropic error ${res.status}`
    try {
...
function streamIt(
  ...
  ;(async () => {
    try {
      ...
      if (msg.includes('529') || msg.toLowerCase().includes('overloaded')) msg = 'Claude is overloaded. Wait 30s and retry.'
      if (msg.includes('401') || msg.toLowerCase().includes('invalid x-api-key')) msg = 'Invalid API key. Check ⚙️ Settings.'
      if (msg.toLowerCase().includes('credit') || msg.toLowerCase().includes('quota')) msg = 'API limit reached. Check console.anthropic.com.'
