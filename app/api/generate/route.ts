...
// Build upcoming posting dates (Tue/Thu/Sat style)
...
// Strip fences and recover JSON even if there is small wrapping text
...
// Low‑level Anthropic streaming call
...
// Stream response back to client; send a keepalive byte immediately
function streamIt(
  key: string,
  model: string,
  system: string,
  prompt: string,
  maxTokens: number,
  validate?: (d: Record<string, unknown>) => void,
): Response {
  ...
  ;(async () => {
    try {
      // Keepalive byte so edge runtime never times out
      ...
      if (msg.includes('529') || msg.includes('overloaded')) {
        msg = 'Claude is overloaded. Wait 30s and retry.'
      }
      if (msg.includes('401')) {
        msg = 'Invalid API key. Check ⚙️ Settings.'
      }
      if (msg.includes('credit') || msg.includes('quota')) {
        msg = 'API limit reached. Check console.anthropic.com.'
      }
...
const SYS =
  'You are a JSON API. Output ONLY valid complete JSON starting with {. Fill ALL fields with real, specific content. Never use placeholder text like "str" or "string". Never truncate.'
...
Reply with ONLY this JSON, filling every field with real specific content:
{
  ...
  "posts": [
    ...
    return streamIt(key, HAIKU, SYS, prompt, maxOut, d => {
      const posts = (d as { posts?: unknown[] }).posts
...
Reply with ONLY this JSON, filling EVERY field with real, specific, actionable content:
{
  ...
  return streamIt(key, SONNET, SYS, prompt, maxTok, d => {
    const posts = (d as { posts?: unknown[] }).posts
    if (!Array.isArray(posts)) {
      throw new Error('No posts returned — try again')
    }
...
export async function POST(req: NextRequest) {
  ...
  const key =
    (body.apiKey as string) || (process.env.ANTHROPIC_API_KEY as string | undefined)

  if (!key) {
    return new Response(
      JSON.stringify({
        error:
          'No API key. Add it in ⚙️ Settings or set ANTHROPIC_API_KEY in Vercel env vars.',
      }),
